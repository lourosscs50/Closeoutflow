...
public sealed class CloseoutRecord
{
    ...
    private CloseoutRecord(Guid id, Guid jobId,  string summary, DateTime createdAtUtc, IEnumerable<ProofItem> proofItems)
    {
        Id = id;
        JobId = jobId;
        Summary = summary;
    ...
    public IReadOnlyCollection<ProofItem> ProofItems => _proofItems;
     public static Result<CloseoutRecord> Create(Guid jobId, string summary, IEnumerable<(ProofItemType Type, string Value)>  proofItems, DateTime createdAtUtc)
    {
        if (jobId == Guid.Empty)
        {
            return Result<CloseoutRecord>.Failure(CloseoutErrors.JobIdRequired);
        }
        var items = proofItems.ToList();
        if (items.Count == 0)
        {
